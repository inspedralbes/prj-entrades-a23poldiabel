<template>
  <div class="event-detail">
    ...
    <div v-else-if="desenvolupament" class="event-content">
      <div class="event-banner" v-if="desenvolupament.imatge">
        <img :src="desenvolupament.imatge" :alt="desenvolupament.nom" />
      </div>
      <header class="event-header">
        <div class="event-date-badge">
          <span class="day">{{ getDay(desenvolupament.data_hora) }}</span>
          <span class="month">{{ getMonth(desenvolupament.data_hora) }}</span>
        </div>
        <div class="event-info">
          <h1>{{ desenvolupament.nom }}</h1>
          <p class="venue">{{ desenvolupament.recinte }}</p>
          <p class="time">{{ getTime(desenvolupament.data_hora) }}</p>
          <p v-if="desenvolupament.descripcio" class="description">{{ desenvolupament.descripcio }}</p>
        </div>
      </header>
      ...
      <div class="zones-legend">
        ...
        <div class="zones-list">
          <div v-for="zona in desenvolupament.zones" :key="zona.id" class="zone-item">
...
const esdevenimentIdActual = ref<string>('');

const carregant = computed(() => eventStore.carregant);
const desenvolupament = computed(() => eventStore.esdevenimentActual);
...
async function carregarEsdeveniment() {
  ...
  try {
    ...
    // Obtenir el desenvolupament
    const esdeveniment = await eventStore.obtenirEsdevenimentPerId(idRuta);
    seientStore.setSeients((esdeveniment as any)?.seients || []);
    esdevenimentIdActual.value = (esdeveniment as any)?.id || idRuta;
    
    // Verificar si hi ha reserva anterior
    const reservesDades = await apiClient.get('/reserves/usuari');
    const existingReserva = ((reservesDades as any).reserves || []).find(
      (r: any) => r.esdeveniment_id === esdevenimentIdActual.value
    );
    ...
    // Unir-se a la sala del event
    if (!esdevenimentIdActual.value) {
      throw new Error('No s\'ha pogut resoldre l\'id intern de l\'esdeveniment');
    }

    joinEvent(esdevenimentIdActual.value);
    
  } catch (e) {
    error.value = e instanceof Error ? e.message : 'Error carregant desenvolupament';
  }
}

// Configurar els handlers de Socket.io
function setupSocketHandlers() {
  ...
  on('seat-selected', (data: any) => {
    ...
    const seientsActualitzats = seientsActuals.some((s) => s.id === data.seient_id)
      ? seientsActuals
      : [...seientsActuals, seientNou];
...
onUnmounted(() => {
  leaveEvent(esdevenimentIdActual.value || (route.params.id as string));
  off('seat-reserved');
  off('seat-selected');
  off('seat-released');
  off('seat-sold');
...
function reservarSeient(seientId: string) {
  const desenvolupamentId = esdevenimentIdActual.value || (route.params.id as string);
  const token = reservaActiva.value?.token;
  const usuariId = authStore.usuari?.id;
  reserveSeat(seientId, desenvolupamentId, token, usuariId);
}
